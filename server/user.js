...
exports.getIdToken = auth.getBearerToken;

exports.userInfo = (req, res) => {
  res.set('Cache-Control', 'no-store');
  return exports.getUserFromToken(exports.getIdToken(req)).then(
    (user) => {
...
exports.paypalIpn = (req, res, next) => Promise.resolve()
  .then(() => {
    ...
    const duration = sponsorship.getDuration(gross);
    if (
    ...
    return fetch(conf.values.paypalUri, {
      ...
      .then((existingUser) => {
        ...
        return result.applied ? exports.putUser(userId, result.user) : undefined;
      })
      .then(() => res.end());
  })
  .catch(next);
